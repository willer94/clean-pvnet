...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        if not self.spherical:
            ...
            vertex_sign_x, vertex_sign_y = vertex_sign[..., 0].transpose(2, 0, 1), vertex_sign[..., 1].transpose(2, 0, 1)
            vertex_abs = vertex_abs.transpose(2, 0, 1)
            ret = {'inp': inp, 'mask': mask.astype(np.uint8), 'vertex_abs': vertex_abs,
                   'vertex_signX': vertex_sign_x,
                   'vertex_signY': vertex_sign_y,
                   'img_id': img_id, 'meta': {}}
